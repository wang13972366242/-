...
@interface ScanPositionV ()<UITableViewDelegate,
...
-(instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(firstDetasoure:) name:@"positionFirstData" object:nil];
    }
    return self;
}

-(void)firstDetasoure:(NSNotification *)notification{
...
-(void)addDetasoure:(NSNotification *)notification{
    ...
    ScanDepartmentModel *model = notification.userInfo[@"ID"];
    _departLabel.text = model.themmeStr;
    ID = [model.ID mutableCopy];
    [ID removeLastObject];
...
-(void)_configurationSubviews:(CGRect)frame{
    ...
    _departLabel = [self creatLabelWithFrame:CGRectMake(0, positionLabelY, selfWight, listLabelH) textAlignment:NSTextAlignmentCenter font:[UIFont systemFontOfSize:14.0] textColor:[UIColor greenColor] text:nil];
    [self addSubview:_departLabel];
