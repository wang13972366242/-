...
@interface WQmanagerVC ()<UITableViewDataSource,UITableViewDelegate,SexViewDelegate,UITextFieldDelegate,SelectedDepartPositionDelegate>
/** tableview*/
...
- (void)viewDidLoad {
    ...
    _data = @[@"用户名:",@"密码:",@"确认密码:",@"邮箱:",@"邮箱验证码:",@"电话:",@"电话验证码:",@"姓名:",@"座机:",@"性别:",@"部门:",@"职位:",@"工号:",@"个人描述:"];
    //创建scrollview
    _scrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 64,KScreenWidth, KScreenHeight - 49 -64 )];
    
    _scrollView.contentSize = CGSizeMake(KScreenWidth,_data.count *40 +80);
    [self.view addSubview:_scrollView];
    //2.创建一个TableView
    [self configurtionTabelV];
    //3.创建"确认"按钮
    [self _configurtionBtn];
    //通知
...
-(void)configurtionTabelV{
    ...
    [self.tableView registerClass:[WQSignPersonCell class] forCellReuseIdentifier:setCell];
    
}

-(void)_configurtionBtn{
    CGFloat BtnW = 60.0;
    CGFloat BtnH = 44.0;
    CGFloat BtnX = (KScreenWidth - BtnW)/2.0;
    CGFloat BtnY = KScreenHeight - BtnH;
    
    _sureBtn =[self creatButtonWithUIButtonType:UIButtonTypeCustom frame:CGRectMake(BtnX, BtnY, BtnW, BtnH) title:@"确认" titleColor:[UIColor blackColor]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    WQSignPersonCell *cell = [[WQSignPersonCell alloc]init];
    if (cell == nil) {
...
-(void)textFieldCell:(WQSignPersonCell *)cell placeholder:(NSString *)placeholder{
...
-(void)memberActivationCode{
    ...
    UILabel *emilCode = _labelArr[4];
    UILabel *mobie = _labelArr[5];
    UILabel *mobieCode = _labelArr[6];
    UILabel *realName = _labelArr[7];
    UILabel *staticnumber = _labelArr[8];
    UILabel *employeeNumber = _labelArr[12];
    UILabel *address = _labelArr[13];
    ...
    @try {
        //1.UserBean
        DeviceUniqueID *DU =  [[DeviceUniqueID alloc]initWithIP:@"175.189.199.12" szMac:@"mac" szOS:@"123" szHostName:@"2134"];
        useb = [[UserBean alloc]initWithszUsername:userNamelabel.text szPassword:password1.text objDeviceID:DU];
    ...
    @try {
        ...
        //2.OrganizedMember
        OrganizedMember *om = [[OrganizedMember alloc]initWithAccount:userNamelabel.text szMobile:mobie.text szEmail:emil.text vldct:CONTACT_MOBILE];
        ...
        messageActiva = [OrganizedClientMessage getInstanceOfUserActivationRequest:uniqueID userBean:useb member:om];
        NSString *messageStr = [messageActiva toString];
        
        checkDic = @{KEY_REQUEST:messageStr,KEY_REQUESTFROM:@"IOS",KEY_ACTVARCODE:varcode,KEY_MOBILEVARCODE:@"465348"};
