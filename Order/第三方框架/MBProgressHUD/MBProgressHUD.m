...
#define MB_TEXTSIZE(text, font) (![text isKindOfClass:NSNull.class] && [text length] > 0) ? [text \
sizeWithAttributes:@{NSFontAttributeName:font}] : CGSizeZero;
#else
#define MB_TEXTSIZE(text, font) (![text isKindOfClass:NSNull.class] && [text length] > 0) ? [text sizeWithFont:font] : CGSizeZero;
#endif

#if __IPHONE_OS_VERSION_MIN_REQUIRED >= 70000
#define MB_MULTILINE_TEXTSIZE(text, font, maxSize, mode) (![text isKindOfClass:NSNull.class] && [text length] > 0) ? [text \
...
- (void)layoutSubviews {
    ...
    CGSize labelSize ;
    if ([label.font isMemberOfClass:[NSNull class]])
    {
        labelSize = MB_TEXTSIZE(label.text, [UIFont systemFontOfSize:17]);
...
//	CGSize labelSize = MB_TEXTSIZE(label.text, label.font);
	...
	CGFloat remainingHeight = bounds.size.height - totalSize.height - padding - 4 * margin;
